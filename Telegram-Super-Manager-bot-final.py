...
async def panel_group_id(call: types.CallbackQuery):
    if not call.message:
        return None
    return panel_sessions.get((call.message.chat.id, call.message.message_id))
...
async def member_economy_panel(call: types.CallbackQuery):
    ...
    await edit_dashboard(call,f'💰 <b>{row[0]}</b>\n⭐ Points: {row[1]}\n🪙 Coins: {row[2]}\n\nاختر العملية:',kb)
...
async def dash_help(call: types.CallbackQuery):
    await call.answer(); await edit_dashboard(call,HELP,InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🔙 الرئيسية',callback_data='dash_home')]]))
