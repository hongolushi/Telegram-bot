...
async def send_private_panel(user_id, group_id):
    ...
    # سجّل صاحب اللوحة في قاعدة البيانات حتى يظهر ضمن قائمة الأعضاء.
    if user_id == OWNER_ID:
        await db_execute("""
            INSERT INTO users(chat_id,user_id,username,name,role)
            VALUES(?,?,?,?,?)
            ON CONFLICT(chat_id,user_id) DO UPDATE SET
            username=excluded.username, name=excluded.name, role='owner'
        """, (group_id, user_id, "", "Owner", "owner"))
...
async def dash_members(call: types.CallbackQuery):
    ...
    gid = await panel_group_id(call)
    # تأكد من وجود الـOwner في قائمة الأعضاء حتى لو لم يرسل رسالة في القروب.
    if call.from_user.id == OWNER_ID:
        await db_execute("""
            INSERT INTO users(chat_id,user_id,name,role) VALUES(?,?,?,?)
            ON CONFLICT(chat_id,user_id) DO UPDATE SET role='owner'
        """, (gid, OWNER_ID, call.from_user.full_name or 'Owner', 'owner'))
    rows=await db_execute('SELECT user_id,name,username,role FROM users WHERE chat_id=? ORDER BY CASE WHEN user_id=? THEN 0 ELSE 1 END, name LIMIT 8 OFFSET ?', (gid, OWNER_ID, page*8), fetch=True)
    buttons=[]
